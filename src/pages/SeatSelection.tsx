...
export default function SeatSelection() {
  ...
  return (
    <div className="container py-6 space-y-4 text-silver bg-slate">
      <Card>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-yellow">{theatre.name}</h2>
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-silver">Tickets:</span>
            {/* Select number of tickets */}
            <select value={count} onChange={e=>{setCount(parseInt(e.target.value)); setSelected([]); setSuggested([])}} className="h-10 border-2 border-yellow bg-matte text-silver rounded-2xl px-3 focus:ring-2 focus:ring-yellow">
      ...
      <Card>
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <div className="inline-block p-2 bg-blue rounded-2xl">
            {/* Render seat grid */}
            {seatGrid.map((row, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-center">
                  {/* Row label */}
                  <span className="w-6 text-xs font-bold text-yellow drop-shadow">{String.fromCharCode(65+i)}</span>
        ...
        <div className="mt-4 flex gap-4 text-sm">
          <div className="flex items-center gap-1"><span className="w-4 h-4 bg-yellow inline-block rounded-sm" />Booked</div>
          <div className="flex items-center gap-1"><span className="w-4 h-4 bg-silver inline-block rounded-sm" />Suggested</div>
          <div className="flex items-center gap-1"><span className="w-4 h-4 bg-matte inline-block rounded-sm" />Selected</div>
          <div className="flex items-center gap-1"><span className="w-4 h-4 bg-matte border border-yellow inline-block rounded-sm" />Free</div>
        </div>
      </Card>
