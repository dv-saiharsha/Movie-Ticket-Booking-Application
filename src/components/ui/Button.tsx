...
const Button = forwardRef<HTMLButtonElement, Props>(({ className, variant='default', size='md', ...props}, ref) => {
  ...
  const variants = {
  default: 'bg-teal text-white hover:bg-darkteal',
  outline: 'border border-teal text-teal bg-cream hover:bg-lightteal',
  ghost: 'bg-transparent text-teal hover:bg-lightteal',
  } as const
